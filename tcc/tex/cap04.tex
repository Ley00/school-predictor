...
\begin{enumerate}
    \item prepara\c{c}\~ao local dos insumos e gera\c{c}\~ao de dados sintéticos compat\'iveis com o contrato p\'ublico;
...
A aplica\c{c}\~ao foi organizada para funcionar em dois cen\'arios complementares. No cen\'ario completo, a rotina parte da prepara\c{c}\~ao local dos insumos, segue para a extra\c{c}\~ao dos CSVs can\^onicos e termina na execu\c{c}\~ao da pipeline com gera\c{c}\~ao de relat\'orios. No cen\'ario p\'ublico reproduz\'ivel, o ponto de partida s\~ao os CSVs j\'a estruturados a partir de dados sintéticos, a partir dos quais os modos anal\'iticos e a recomposi\c{c}\~ao da camada final podem ser executados sem acesso a dados institucionais.
...
\begin{itemize}
    \item camada privada local: prepara insumos locais, quando necess\'ario, e concentra detalhes operacionais que n\~ao fazem parte da vers\~ao p\'ublica;
...
\begin{itemize}
    \item \texttt{aluno.csv}: identifica o aluno por registros artificiais consistentes com o contrato p\'ublico e preserva o contexto cadastral e escolar b\'asico da extra\c{c}\~ao;
...
No cen\'ario completo, dispon\'ivel apenas em ambiente local com acesso controlado aos insumos de prepara\c{c}\~ao, a rotina segue tr\^es movimentos:
\begin{enumerate}
    \item preparar localmente os insumos por meio da rotina \texttt{prepare-db};
...
Do ponto de vista de engenharia, a entrada de dados foi organizada em duas camadas. A primeira camada \'e privada e local, respons\'avel pela prepara\c{c}\~ao dos insumos e pela gera\c{c}\~ao dos arquivos necess\'arios \`a vers\~ao p\'ublica. A segunda camada \'e p\'ublica e reproduz\'ivel, baseada nos arquivos CSV can\^onicos consumidos pela pipeline em \path{artifacts/database/csv/}. Isso permite explicar e reproduzir a arquitetura sem depender de dados institucionais identific\'aveis.
...
Do ponto de vista metodol\'ogico, a reprodu\c{c}\~ao desta solu\c{c}\~ao foi organizada com base numa interface p\'ublica de dados de entrada, formada pelos ficheiros can\^onicos conservados em \path{artifacts/database/csv/}. Esta escolha possibilita distinguir a fase institucional de extra\c{c}\~ao, que requer uma base de dados restaurada e procedimentos privados locais, da fase acad\^emica de reprodu\c{c}\~ao, que pode ser executada a partir de ficheiros tabulares j\'a consolidados e definidos semanticamente. Em termos pr\'aticos, a extra\c{c}\~ao institucional continua associada \`a camada privada, enquanto a reprodu\c{c}\~ao acad\^emica passa a ter in\'icio, de forma adequada, a partir do contrato destes CSVs. A vers\~ao p\'ublica reproduz\'ivel dessa arquitetura est\'a disponibilizada no reposit\'orio oficial do projeto no GitHub \cite{schoolpredictorgithub}, que concentra o c\'odigo, a documenta\c{c}\~ao e a organiza\c{c}\~ao necess\'arias para consulta e replica\c{c}\~ao acad\^emica.

Na estrutura divulgada, os pontos mais imediatos de acesso a esses dados est\~ao na configura\c{c}\~ao do fluxo de trabalho e na importa\c{c}\~ao das tabelas fundamentais. Assim, para reproduzir a solu\c{c}\~ao em seu formato p\'ublico, o ponto de partida recomendado \'e o reposit\'orio citado, em conjunto com os CSVs can\^onicos esperados pela pipeline.

Na pr\'atica, a configura\c{c}\~ao da pipeline define os caminhos esperados para os CSVs can\^onicos da solu\c{c}\~ao. Em seguida, a camada de leitura realiza a importa\c{c}\~ao padronizada dessas fontes e organiza a carga preliminar para fins anal\'iticos. Depois disso, a camada de orquestra\c{c}\~ao incorpora essa carga ao fluxo principal. J\'a a rotina privada de banco permanece restrita \`a camada local, respons\'avel pela prepara\c{c}\~ao da base restaurada e pela extra\c{c}\~ao anterior ao processamento geral.
...
\hline
\makecell{\textbf{IDAluno}} & \makecell{\textbf{Desc.}\\\textbf{Mov.}} & \makecell{\textbf{Venc.}\\\textbf{Mov.}} & \makecell{\textbf{Valor}\\\textbf{Mov.}} & \makecell{\textbf{Pago?}} & \makecell{\textbf{Mensal.?}} & \makecell{\textbf{Periodo}} \\
\hline
A1001 & Mensal. Mar. & 2025-03-05 & 850.0 & Sim & Sim & 2025 \\
\hline
\end{tabular}}
\renewcommand{\arraystretch}{1}
\normalsize
