...
A arquitetura final foi organizada para separar claramente as responsabilidades do sistema. Essa organiza\c{c}\~ao n\~ao surgiu pronta desde o in\'icio; ela foi sendo consolidada \`a medida que o projeto deixou de ser apenas um conjunto de experimentos e passou a exigir uma estrutura reproduz\'ivel, compreens\'ivel e segura para publica\c{c}\~ao acad\^emica. O fluxo consolidado pode ser resumido em oito etapas:
...
Na vers\~ao final do reposit\'orio, essa organiza\c{c}\~ao foi consolidada no pacote \texttt{school\_predictor/}, com artefatos locais centralizados em \texttt{artifacts/}. Essa separa\c{c}\~ao tornou a arquitetura mais simples de compreender e mais segura para publica\c{c}\~ao do c\'odigo. Em especial, ela ajudou a distinguir com clareza o que pertence \`a camada sens\'ivel de prepara\c{c}\~ao dos dados e o que pode ser mostrado, executado e auditado no contexto do TCC.

A Figura~\ref{fig:arquitetura-solucao} sintetiza essa organiza\c{c}\~ao em camadas maiores, priorizando a leitura do caminho dos dados e das responsabilidades principais da solu\c{c}\~ao. Em vez de detalhar todos os arquivos da aplica\c{c}\~ao, o diagrama destaca cinco blocos centrais: opera\c{c}\~ao e interface, camada privada local, camada p\'ublica reproduz\'ivel, artefatos finais e uso institucional. O objetivo aqui n\~ao \'e detalhar a sequ\^encia de execu\c{c}\~ao, mas mostrar de forma mais direta onde cada responsabilidade ficou concentrada na vers\~ao final do projeto.
...
Essa divis\~ao surgiu dos experimentos: o mesmo recorte de hist\'orico n\~ao era o melhor para os dois objetivos. Assim, a arquitetura passou a preservar duas configura\c{c}\~oes operacionais, cada uma adequada ao seu uso principal. Em vez de for\c{c}ar uma solu\c{c}\~ao \'unica para tarefas diferentes, o projeto assumiu explicitamente essa diferen\c{c}a como parte do desenho final.

Em ambos os modos, a fun\c{c}\~ao central de modelagem calcula sinais de regress\~ao e de classifica\c{c}\~ao. A diferen\c{c}a est\'a no objetivo operacional, no hist\'orico m\'inimo exigido, no volume de dados resultante e no artefato usado como refer\^encia principal. No modo \texttt{previsao\_nota}, a interpreta\c{c}\~ao principal \'e o erro da nota prevista; no modo \texttt{alerta\_risco}, a interpreta\c{c}\~ao principal \'e a capacidade de recuperar alunos cuja pr\'oxima nota ficou abaixo da linha de seguran\c{c}a. Essa distin\c{c}\~ao ajuda a evitar uma leitura simplificada de que a pipeline executa exatamente a mesma coisa duas vezes.

\section{Configura\c{c}\~ao experimental e reprodutibilidade}
\label{sec:config-experimental}

Para fins de reprodutibilidade, a configura\c{c}\~ao experimental final do projeto foi organizada como um contrato entre entradas can\^onicas, rotinas de execu\c{c}\~ao e artefatos gerados. A ideia central foi permitir que a arquitetura p\'ublica do TCC pudesse ser compreendida, auditada e reexecutada sem expor o banco institucional nem as consultas SQL reais. Essa se\c{c}\~ao, portanto, n\~ao funciona apenas como invent\'ario de arquivos: ela sintetiza a forma como a solu\c{c}\~ao foi estabilizada para uso acad\^emico.

A Figura~\ref{fig:fluxo-pipeline} resume esse percurso operacional completo: mostra como os comandos principais partem da interface de linha de comando, acionam a prepara\c{c}\~ao ou a extra\c{c}\~ao quando necess\'ario e, no fluxo principal, convergem para a constru\c{c}\~ao do dataset temporal, para a execu\c{c}\~ao dos modos \texttt{previsao\_nota} e \texttt{alerta\_risco} e para a consolida\c{c}\~ao final dos relat\'orios escolares.
...
Essa separa\c{c}\~ao foi adotada para que a parte sens\'ivel da extra\c{c}\~ao permanecesse protegida, enquanto a parte acad\^emica e demonstrativa da solu\c{c}\~ao continuasse pass\'ivel de reprodu\c{c}\~ao por terceiros. Na pr\'atica, ela permitiu conciliar duas exig\^encias que normalmente entram em tens\~ao em projetos escolares: proteger a estrutura institucional e, ao mesmo tempo, manter a rastreabilidade do experimento.
...
Essa organiza\c{c}\~ao foi importante porque o experimento final n\~ao depende de acesso direto ao banco para ser reproduzido academicamente; ele depende do contrato dos \texttt{CSV}s e de uma interface de execu\c{c}\~ao padronizada. Em outras palavras, a reprodutibilidade do trabalho foi deslocada do banco institucional para a interface p\'ublica dos dados extra\'idos.
...
Esses artefatos sustentam a leitura t\'ecnica do experimento, pois registram o tamanho da base resultante, os candidatos comparados, o desempenho temporal, as predi\c{c}\~oes no teste e os pontos de maior dificuldade do sistema. Eles tamb\'em ajudam a separar a fase de modelagem da fase de consumo institucional dos resultados.
...
Esses relat\'orios representam a sa\'ida final do experimento no contexto do TCC, pois convertem resultados de modelagem em materiais de consulta orientados aos diferentes perfis institucionais da escola. Esse ponto \'e decisivo para o sentido do trabalho: a entrega n\~ao \'e apenas um modelo treinado, mas uma camada de sa\'ida pensada para uso real.
